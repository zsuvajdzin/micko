...
class SymbolTable(object):
    ...
    def lookup_symbol(self, sname, skind=kinds.keys(), stype=types.keys()):
    ...
    def insert_local_var(self, vname, vtype, position):
        ...
        index = self.insert_id(vname, self.kinds.LOCAL_VAR, [self.kinds.LOCAL_VAR, self.kinds.PARAMETER], vtype)
...
class CodeGenerator(object):
    ...
    RELATIONAL_DICT = dict([op,i] for i, op in enumerate(RELATIONAL_OPERATORS))
    ...
    def arithmetic(self, operator, operand1, operand2, operand3 = None):
    ...
    def relop_code(self, relop, typ):
        """Returns code for relational operator
           relop    - relational operator
           typ      - operands type (int or unsigned)
        """
        code = self.RELATIONAL_DICT[relop]
        offset = 0 if typ == SymbolTable.types.INT else len(self.RELATIONAL_OPERATORS)
        return code + offset
...
class MicroC(object):
    ...
    def __init__(self):
        ...
        self.tRelOp = oneOf(CodeGenerator.RELATIONAL_OPERATORS)
        self.tMulOp = oneOf("* /")
        self.tAddOp = oneOf("+ -")
        ...
        self.rWhileStatement = Group((Keyword("while") + FollowedBy("(")).setParseAction(self.while_begin_action) +
    ...
    def local_variable_action(self, text, loc, var):
        ...
        if DEBUG > 0:
            ...
            if DEBUG > 9: return
        index = self.symtab.insert_local_var(var.name, var.type, self.compiler.function_vars)
    ...
    def function_return_action(self, text, loc, ret):
        ...
        self.codegen.jump(CodeGenerator.UNCONDITIONAL, False, self.codegen.label(self.compiler.function_name+"_exit", True))
    ...
    def andexp_action(self, text, loc, arg):
        self.compiler.setpos(loc,text)
        if DEBUG > 0:
            print "ANDEXP:",arg, self.relexp_code, arg.relexp
            if DEBUG > 9: return
    ...
    def logexp_action(self, text, loc, arg):
        self.compiler.setpos(loc,text)
        if DEBUG > 0:
            print "LOGEXP:",arg, self.andexp_code
            if DEBUG > 9: return
...
#    RELATIONAL_OPERATORS = "< > <= >= == !=".split()
    ...
    def if_body_action(self, text, loc, arg):
        self.compiler.setpos(loc,text)
        if DEBUG > 0:
            print "IFBODY:",arg, self.relexp_code
            if DEBUG > 9: return
        label = self.codegen.label("false{0}".format(self.false_label_number), True, False)
        self.codegen.jump(self.relexp_code, True, label)
    ...
    def if_else_action(self, text, loc, arg):
        ...
        self.codegen.jump(CodeGenerator.UNCONDITIONAL, True, label)
    ...
    def while_body_action(self, text, loc, arg):
        self.compiler.setpos(loc,text)
        if DEBUG > 0:
            print "WHILEBODY:",arg, self.relexp_code
            if DEBUG > 9: return
        label = self.codegen.label("false{0}".format(self.false_label_number), True, False)
        self.codegen.jump(self.relexp_code, True, label)
        self.codegen.newline_label("true{0}".format(self.label_number), True, True)
