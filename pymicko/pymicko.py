...
#    pyparsing educational microC compiler v1.0
#    Copyright (C) 2009  Zarko Zivanov
#    (based on flex/bison microC compiler by Zorica Suvajdzin, used with her permission)
#    (microC grammar and flex/bison implementation can be found at
#     <http://www.acs.uns.ac.rs/sr/repozitorijum?path=/Predmeti/Programski%20Prevodioci>)
#
...
# (small subset of C made for compiler course at Faculty of Technical Sciences, Novi Sad, Serbia)
# 
# Patterns:
# 
# letter
...
class SharedData(object):
    ...
    #constant for unconditional jump
    UNCONDITIONAL = -1

    def __init__(self):
        #index of currently parsed function
        self.functon_index = 0
        #name of currently parsed function
        self.functon_name = 0
        #number of parameters of currently parsed function
        self.function_params = 0
        #number of local variables of currently parsed function
...
class SymbolTable(object):
    ...
    def __init__(self, shared):
        ...
        self.table = []
        self.lable_len = 0
        #put working registers in symbol table
    ...
    def error(self, text=""):
        """Symbol table error exception. It should happend only if something is wrong with accessing symbol table.
           This exeption is not handled by compiler, to allow traceback printing
        """
        if text == "":
            raise Exception("Symbol table index out of range" % text)
        else:
            raise Exception("Symbol table error: %s" % text)

    def display(self):
        """Displays symbol table content."""
        #Finding maximum length for each column
    ...
    def same_type_as_argument(self, index, function_index, argument_number):
        """Returns True if function's argument and any index are of the same type
...
class CodeGenerator(object):
    ...
    #suffixes for signed and unsigned operations (if no type is specified, it will be assumed unsigned)
    ...
    def __init__(self, shared, symtab):
        ...
        self.used_registers = []
        #list of used registers needed when function call is inside a function call
    ...
    def error(self, text):
        """Compiler error exception. It should happend only if something is wrong with compiler.
           This exeption is not handled by compiler, to allow traceback printing
        """
    ...
    def move(self,operand1, operand2):
        """Generates move operation
           If output operand (opernad2) is working register, sets it's type
           operandX - index in symbol table or text representation of operand
        """
        if isinstance(operand1, int):
            output_type = self.symtab.get_type(operand1)
    ...
    def relop_code(self, relop, operands_type):
        """Returns code for relational operator
           ...
           operands_type - constants for int or unsigned
        """
    ...
    def jump(self, relcode, opposite, label):
        """Generates jump operation
           ...
           label    - jump label
        """
        if relcode == SharedData.UNCONDITIONAL:
            jump = "JMP "
        else:
...
class MicroC(object):
    ...
    def __init__(self):
        ...
        #Definitions of rules for logic expressions (these are without parenthesis support)
        ...
        #index of current function call
        self.function_call_index = -1
        #stack for nested function calls
        self.function_call_stack = []
        #arguments of current function call
        self.function_arguments = []
        #stack for arguments of nested function calls
        self.function_arguments_stack = []
        #number of arguments for curent function call
        self.function_arguments_number = -1
        #stack for number of arguments for nested function calls
    ...
    def global_variable_action(self, text, loc, var):
        """Code executed after recognising global variable"""
    ...
    def local_variable_action(self, text, loc, var):
        """Code executed after recognising local variable"""
        exshared.setpos(loc, text)
        if DEBUG > 0:
    ...
    def parameter_action(self, text, loc, par):
        """Code executed after recognising parameter"""
        exshared.setpos(loc, text)
        if DEBUG > 0:
            print "PARAM:",par
            if DEBUG == 2: self.symtab.display()
    ...
    def constant_action(self, text, loc, const):
        """Code executed after recognising constant"""
        exshared.setpos(loc, text)
        if DEBUG > 0:
            print "CONST:",const
            if DEBUG == 2: self.symtab.display()
    ...
    def function_begin_action(self, text, loc, fun):
        """Code executed after recognising function definition (type and function name)"""
        exshared.setpos(loc, text)
        if DEBUG > 0:
            print "FUN_BEGIN:",fun
            if DEBUG == 2: self.symtab.display()
    ...
    def function_body_action(self, text, loc, fun):
        """Code executed after recognising beginning of function's body"""
        exshared.setpos(loc, text)
        if DEBUG > 0:
            print "FUN_BODY:",fun
            if DEBUG == 2: self.symtab.display()
    ...
    def return_action(self, text, loc, ret):
        """Code executed after recognising return statement"""
        exshared.setpos(loc, text)
        if DEBUG > 0:
            print "RETURN:",ret
            if DEBUG == 2: self.symtab.display()
        ...
        #set refister for function's return value to expression value
        ...
        self.codegen.jump(SharedData.UNCONDITIONAL, False, self.codegen.label(self.shared.function_name+"_exit", True))

    def lookup_id_action(self, text, loc, var):
        """Code executed after recognising identificator in expression"""
        exshared.setpos(loc, text)
        if DEBUG > 0:
            print "EXP_VAR:",var
            if DEBUG == 2: self.symtab.display()
    ...
    def assignment_action(self, text, loc, assign):
        """Code executed after recognising assignment statement"""
        exshared.setpos(loc, text)
        if DEBUG > 0:
            print "ASSIGN:",assign
            if DEBUG == 2: self.symtab.display()
        ...
        if var_index == None:
            raise SemanticException("Invalid lvalue '%s' in assignment" % assign.var)
    ...
    def mulexp_action(self, text, loc, mul):
        """Code executed after recognising mulexp expression (something *|/ something)"""
        exshared.setpos(loc, text)
        if DEBUG > 0:
            print "MUL_EXP:",mul
            if DEBUG == 2: self.symtab.display()
        ...
        while len(m) > 1:
            ...
            #replace first calcualtion with it's result
            m[0:3] = [reg]
        return m[0]
    ...
    def function_call_prepare_action(self, text, loc, fun):
        """Code executed after recognising function call (type and function name)"""
        exshared.setpos(loc, text)
        if DEBUG > 0:
            print "FUN_PREP:",fun
            if DEBUG == 2: self.symtab.display()
    ...
    def function_call_action(self, text, loc, fun):
        """Code executed after recognising whole function call"""
        exshared.setpos(loc, text)
        if DEBUG > 0:
            print "FUN_CALL:",fun
            if DEBUG == 2: self.symtab.display()
        ...
        if len(self.function_arguments) != self.symtab.get_attribute(self.function_call_index):
            raise SemanticException("Wrong number of arguments to function '%s'" % fun.name)
        ...
        #move result to new free register, to allow next function call
        self.codegen.move(self.codegen.take_function_register(return_type), register)
        return register

    def relexp_action(self, text, loc, arg):
        """Code executed after recognising relexp expression (something relop something)"""
        if DEBUG > 0:
            print "REL_EXP:",arg
            if DEBUG == 2: self.symtab.display()
            if DEBUG > 2: return
        ...
        if not self.symtab.same_types(arg[0], arg[2]):
            raise SemanticException("Invalid operands to '{0}'".format(arg[1]))
    ...
    def andexp_action(self, text, loc, arg):
        """Code executed after recognising mulexp expression (something and something)"""
        exshared.setpos(loc, text)
        if DEBUG > 0:
            print "AND+EXP:",arg
            if DEBUG == 2: self.symtab.display()
    ...
    def logexp_action(self, text, loc, arg):
        """Code executed after recognising mulexp expression (something or something)"""
        exshared.setpos(loc, text)
        if DEBUG > 0:
            print "LOG_EXP:",arg
            if DEBUG == 2: self.symtab.display()
    ...
    def if_begin_action(self, text, loc, arg):
        """Code executed after recognising if statement (if keyword)"""
        exshared.setpos(loc, text)
        if DEBUG > 0:
            print "IF_BEGIN:",arg
            if DEBUG == 2: self.symtab.display()
    ...
    def if_else_action(self, text, loc, arg):
        ...
        self.codegen.jump(SharedData.UNCONDITIONAL, True, label)
    ...
    def if_end_action(self, text, loc, arg):
        """Code executed after recognising whole if statement"""
        exshared.setpos(loc, text)
        if DEBUG > 0:
            print "IF_END:",arg
            if DEBUG == 2: self.symtab.display()
    ...
    def while_begin_action(self, text, loc, arg):
        """Code executed after recognising while statement (while keyword)"""
        exshared.setpos(loc, text)
        if DEBUG > 0:
    ...
    def while_end_action(self, text, loc, arg):
        """Code executed after recognising whole while statement"""
        exshared.setpos(loc, text)
        if DEBUG > 0:
            print "WHILE_END:",arg
            if DEBUG == 2: self.symtab.display()
    ...
    def program_end_action(self, text, loc, arg):
        """Checks if there is a 'main' function and type of 'main' function"""
        exshared.setpos(loc, text)
        if DEBUG > 0:
        ...
        if index == None:
            ...
            raise SemanticException("Return type of 'main' is not int", False)
...
If input file is omitted, stdin is used
    """.format(argv[0])
    print usage
    exit(1)
try:
